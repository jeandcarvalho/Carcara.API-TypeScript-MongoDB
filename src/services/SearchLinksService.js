    ...
    static search(rawUrl) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const oMaxspeedRaw = splitList(q.get('o.maxspeed'));
                const oMaxspeed = oMaxspeedRaw.map(x => (x.includes('.') ? x : `${x}.0`));
                ...
                const sBuildingRange = sBuildingTok ? (() => {
                    // Primeiro tenta interpretar como range explícito (ex.: "10..30", "..50", "20..")
                    const parsed = parseRange(sBuildingTok);
                    if (parsed)
                        return parsed;
                    // Mantém compatibilidade com low/mid/high
                ...
                const sVegetationRanges = sVegetationTok ? (() => {
                ...
                const yConfRange = parseRange(q.get('y.conf'));
                const yDistRange = parseRange(q.get('y.dist_m'));
                // Extensões aceitas
                ...
                const yoloActive = yClasses.length > 0 || yRel.length > 0 || !!yConfRange || !!yDistRange;
                ...
                // yolo_1hz
                if (yClasses.length > 0 || yRel.length > 0 || yConfRange || yDistRange) {
                ...
                if (overpassActive) {
                    ...
                    const rows = yield prisma.overpass_1hz.findMany({
                        where: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ acq_id: { in: acqUniverse } }, (highwayVals.size ? { highway: { in: [...highwayVals] } } : {})), (landuseVals.size ? { landuse: { in: [...landuseVals] } } : {})), (oLanes.length ? { lanes: { in: oLanes } } : {})), (oMaxspeed.length ? { maxspeed: { in: oMaxspeed } } : {})), (oOneway.length ? { oneway: { in: oOneway } } : {})), (oSidewalk.length ? { sidewalk: { in: oSidewalk } } : {})), (oCycleway.length ? { cycleway: { in: oCycleway } } : {})),
                        select: {
                            acq_id: true,
                            sec: true,
                            highway: true,
